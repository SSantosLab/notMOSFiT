...
class Diffusion(Transform):
	...
	# Chracteristic time  (Eqn 19 in Arnett 1982, Eqn 6 in Nicholl et al)
	# Photon diffusion time, depends on characteristics of matter
        self._tau_diff = np.sqrt(self.DIFF_CONST * self._kappa *
                                 self._m_ejecta / self._v_ejecta) / DAY_CGS

	# "Leakage parameter, A" (Eqn 7 Nicholl et al.) 
	# The leakage term (1 - e(-At^2)) controls the fraction of the magnetar
	# input that that is thermalized in the ejecta
        self._trap_coeff = (
	...
	# Initialize new luminosities array, which will hopefully return Eqn. 5
	# in Nicholl et al.
	...
	# Okay so it calculates the luminosity as prescribed in Eq 5 of Nicholl
	# et al for some subset of the luminosities? (Unclear, needs analysis.)
	
	# np.expm1 is exp(x) -1 (the eqn is 1-exp(x) but the - is thrown into
	# the -2.
