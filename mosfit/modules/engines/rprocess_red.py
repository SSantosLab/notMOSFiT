...
class RProcess(Engine):
    ...
    _REFERENCES = [
        {SOURCE.BIBCODE: '2016ApJ...829..110B'},
	...
	# Barnes simulation results from 2016ApJ...829..110B (Barnes and Kasen)
	# v = ejecta velocity, M = ejecta mass
        barnes_v = np.asarray([0.1, 0.2, 0.3])
        barnes_M = np.asarray([1.e-3, 5.e-3, 1.e-2, 5.e-2])
	
	# a,b,d are parameterizations of the overall thermalization efficency
	# laid out in 2016ApJ...829..110B: Eqn. 34
	
	# The numbers given here are for simulation results in
	# 2016ApJ...829..110B: Table 1
	...
	# To get values for others in between, we interpolate
    ...
    def process(self, **kwargs):
        """Process module."""
	# To get luminosity out, pass a times array, a mass, a t_explosion, and a
	# v_ejecta
        ...
        self._phi = kwargs[self.key('phi')] # 1/2 opening angle
        self._theta = kwargs[self.key('theta')] # viwing  angle
        self._weight =1. - ( 2*np.cos(self._theta)*(1. - np.cos(self._phi)))
 
	# These are the interpolator functions interpoalted for a, 2b, and d
	...
	# This is 2017LRR....20....3M: Eqn 22, where the Barnes functions are
	# implemented as e_th (thermalization efficency)
	luminosities = [
         self._weight *   self._lscale * (0.5 - IPI * np.arctan(
